...
function createChart(canvasId, title, labels) {
    ...
    // Verificar si es una de las gráficas excluidas
    const excludedCharts = ['ruido', 'barometro', 'vibracion'];

    // Condición para agregar o no la línea 'ind_'
    ...
    return new Chart(ctx, {
        ...
        data: {
            ...
            datasets: labels.map(label => ({
                ...
                tension: 0.1,
                fill: false,
                data: []  // Datos para las variables originales
            }))
            // Si la gráfica no está excluida, añadimos la línea 'ind_'
            .concat(includeIndLine ? [{
                label: `ind_${canvasId.replace('Chart', '')}`,  // Nombre de la nueva línea, como 'ind_mag'
                borderColor: 'rgba(255, 0, 0, 0.8)',  // Color rojo para la nueva línea
                borderWidth: 2,
                tension: 0.1,
                fill: false,
                data: []  // Datos de la nueva línea, que vamos a llenar con valores calculados
            }] : [])
        },
        options: {
            responsive: true,
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            ...
                            // Si es la línea 'ind_', mostrar Min y Max
                            ...
                            // Mostrar las estadísticas para las otras líneas
                            if (stats) {
                                return [
                                    `${label}: ${value.toFixed(3)}`,
...
async function fetchData() {
    try {
        ...
        const currentGroup = data.find(group => group.averages.id === currentId);
        if (currentGroup) {
            updateCharts(currentGroup.averages);
            statistics = currentGroup.statistics;
        }
        currentId = (currentId % data.length) + 1;
    } catch (error) {
...
document.addEventListener('DOMContentLoaded', function() {
    initCharts();
    initStandardizedChart();  // Inicializa la gráfica estandarizada
    fetchData();
